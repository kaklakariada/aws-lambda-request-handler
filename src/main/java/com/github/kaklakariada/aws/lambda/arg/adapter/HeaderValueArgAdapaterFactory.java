...
public class HeaderValueArgAdapaterFactory extends ArgAdapterFactory {
	...
	public Class<? extends Annotation> getSupportedArgAnnotation() {
		return HeaderValue.class;
	}

	@Override
	public SingleArgValueAdapter createAdapter(Parameter param) {
		final String parameterName = param.getAnnotation(HeaderValue.class).value();
		return (ApiGatewayRequest request, Context context) -> request.getHeaders().get(parameterName);
	}
}
